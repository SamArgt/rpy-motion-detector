...
import time
import shutil
from .frame_detection import draw_contour
...
class MotionDetector:
    ...
    def detect_motion(self, frame, contours):
        ...
        if (
            ...
            self.handle_motion_detection(frame, trigger_contour)
    ...
    def handle_motion_detection(self, frame, trigger_contour):

        if not self.is_event_ongoing:
            self.start_event(frame, trigger_contour)

        if not self.is_movie_recording and not self.config.movie.enable:
            self.start_movie_recording()

    def start_event(self, frame, trigger_contour):
        ...
        if self.config.picture.enable:
            self.take_picture(frame, contour=trigger_contour)
    ...
    def take_picture(self, frame, contour=None):
